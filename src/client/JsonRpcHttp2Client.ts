import http2, { type OutgoingHttpHeaders } from 'http2';
...
export class JsonRpcHttp2Client<
...
> extends JsonRpcClient<Sch> {
    ...
    async #post(url: string | URL, request: ReadableStream<string>) {
        ...
        try {
            ...
            const headers = (await headerPromise) as Record<
                string,
                string | string[]
            >;
            const status = Number(
                headers[HTTP2_HEADER_STATUS] instanceof Array
